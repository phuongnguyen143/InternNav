...
LatentEmbSize = 768
MODEL_PATH_TO = "/home/lenguyen1/hoangpqn/vln/InternNav/checkpoints"
...
class MemoryEncoder(nn.Module):
    def __init__(
        self, hidden_size=384, num_heads=6, num_layers=3, max_len=512, dropout=0.1
    ):
        super().__init__()
...
class QFormer(nn.Module):
    ...
    def forward(self, visual_feats, visual_attn_mask=None):
        ...
        out = self.decoder(
            query_tokens, visual_feats, memory_key_padding_mask=visual_attn_mask
        )
        return out
...
class InternVLAN1MetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "system1"):
            self.latent_queries = nn.Parameter(
                torch.randn(1, config.n_query, config.hidden_size)
            )
            # Learnable scale for vision-patch pixel-goal grounding (sigmoid -> ~0.5 at init).
            self.pixel_goal_grounding_weight = nn.Parameter(torch.tensor(0.0))
    ...
    def initialize_vision_modules(self, model_args):
        ...
        if getattr(self, "latent_queries", None) is None:
            ...
            self.latent_queries = nn.Parameter(
                torch.randn(1, self.config.n_query, self.config.hidden_size)
            )
        if getattr(self, "pixel_goal_grounding_weight", None) is None:
            self.pixel_goal_grounding_weight = nn.Parameter(torch.tensor(0.0))
...
class InternVLAN1MetaForCausalLM(ABC):
    ...
    def get_sigmas(self, timesteps, device, n_dim=4, dtype=torch.float32):
        ...
        schedule_timesteps = self.get_model().noise_scheduler.timesteps.to(
            device=device
        )
