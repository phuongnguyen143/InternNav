...
# Local torchrun launcher for train_system2.sh.
...
Train InternVLA-N1 System2 locally with torchrun.
...
Options:
  ...
  --model-path PATH       Base Qwen2.5-VL model path. Default: ${MODEL_PATH}
  ...
  --vision-tower-lr VALUE Vision tower learning rate. Default: ${VISION_TOWER_LR}
...
Examples:
  ...
  $0 --model-path /path/to/Qwen2.5-VL-7B-Instruct --no-deepspeed
EOF
}
...
if [[ ! -e "${MODEL_PATH}" ]]; then
    echo "Warning: base model path does not exist locally: ${MODEL_PATH}"
    echo "         Pass --model-path PATH if your Qwen2.5-VL weights live elsewhere."
fi
...
echo "Starting local InternVLA-N1 System2 training"
