...
from trajectory_io import FloorEntry
...
def smooth_bspline(
    ...
    if n < 4:
        print(
            f"[smooth] bspline needs >= 4 points (got {n}); "
            f"falling back to moving_average window={fallback_window}"
        )
...
def _draw_compare_paths(
    ...
    *,
    ...
    # After first (underneath), then before on top with per-pose dots.
    plot_ax.plot(ax, ay, color="#4fc3f7", linewidth=1.0, alpha=0.9, zorder=2, label="after smooth")
    plot_ax.plot(
        bx, by, color="#ff8a65", linewidth=1.0, linestyle="--", alpha=0.95, zorder=3, label="before smooth"
    )
    ...
    if zoom_rect is not None:
        ...
        plot_ax.add_patch(
            Rectangle(
                ...
                linewidth=1.5, edgecolor="yellow", facecolor="none", linestyle="--", zorder=6,
            )
        )
...
def _save_static_comparison(
    ...
    *,
    ...
    _draw_compare_paths(
        ax_over, bx, by, ax, ay,
        xlim=u_over, ylim=v_over,
        title=f"Overview ({label})",
        zoom_rect=zoom_rect,
    )
    _draw_compare_paths(
        ax_zoom, bx, by, ax, ay,
        xlim=u_zoom, ylim=v_zoom,
...
def draw_smooth_comparison(
    ...
    *,
    ...
    """
    if interactive:
        show_smooth_comparison_interactive(
            before, after, method, window=window, bspline_s=bspline_s
        )
